...
def _mmd_with_optional_bootstrap(ref_desc, gen_desc, *, metric_kind: str, sigma: float | None, num_bootstrap: int, seed: int):
    ...
    # Keep the full-sample estimate as the reported metric and use bootstrap only
    # for uncertainty.  This avoids silently replacing the point estimate with a
    # resampling mean.
...
def _evaluate(args, *, seed: int, output_path: Path | None) -> dict:
    ...
    ref_graphs = _subsample(_load_reference_graphs(args.dataset, args.dataset_root, args.reference_split), max_ref_graphs, args.seed)
...
def main() -> None:
    ...
    parser.add_argument("--max-graphs", type=int, default=None, help="Backward-compatible cap applied to both reference and generated graphs unless a side-specific cap is supplied.")
